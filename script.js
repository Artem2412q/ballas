(() => {
  ...
  // Allow overriding characters list from database (Firestore) before this script loads.
  // characters/index.html sets window.__CHARACTERS_OVERRIDE and then injects script.js.
  let CHARACTERS = (window.__CHARACTERS_OVERRIDE && Array.isArray(window.__CHARACTERS_OVERRIDE))
    ? window.__CHARACTERS_OVERRIDE
    : [
    {
      id: 'vincent-carter',
      name: 'Vincent Carter',
  ...
  function renderCharacters(containerId, opts = {}) {
    ...
    if (!list.length) {
      ...
      const err = window.__CHARACTERS_DB_ERROR;
      grid.innerHTML = `
        <div class="panel reveal" style="grid-column: 1 / -1">
          <div class="panel__head">
            <div>
              <div class="panel__title">Состав пока пустой</div>
              <div class="panel__sub">создай персонажа в профиле — и он появится здесь</div>
            </div>
            <span class="badge">db</span>
          </div>
          <p class="p">
            ${err ? 'Похоже, нет доступа к базе или правила Firestore закрыты. Войди в аккаунт и проверь правила чтения.' : 'Зайди в «Профиль», заполни анкету и сохрани. После этого карточка появится в составе автоматически.'}
          </p>
          <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:12px">
            <a class="btn btn--primary" href="${root}/profile.html">Открыть профиль</a>
            <a class="btn btn--ghost" href="${root}/login.html">Вход</a>
          </div>
        </div>
      `;
      observeReveals();
      ...
      return `
        <article id="char-${esc(c.id)}" class="card reveal" data-role="${esc(c.role)}" data-tags="${esc(c.tags.join(' '))}">
          <div class="cardTop">
            <div>
              <div class="h3">
                <a class="link" href="${rjoin(`person.html?slug=${encodeURIComponent(c.slug || slugify(c.name))}`)}">${esc(c.name)}</a>
                <span class="small">${esc(c.alias)}</span>
              </div>
